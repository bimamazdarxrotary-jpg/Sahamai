...
// ══════════════════════════════════════════════════════════════════
console.log('\n📊 analyzeBandar — Extended Coverage');
// ══════════════════════════════════════════════════════════════════

test('Distribution trap menurunkan bandarScore', () => {
  const base = makeCandles(25, { basePrice: 1000, priceDrift: 5, baseVol: 500000 });
  // Inject distribution candles
  base[22] = makeLongUpperWick(1110, 1200000);
  base[23] = makeLongUpperWick(1115, 1300000);
  base[24] = makeLongUpperWick(1120, 1400000);
  const r1 = analyzeBandar(makeCandles(25), {}, {}, {}, {});
  const r2 = analyzeBandar(base, {}, {}, {}, {});
  assert(r2.bandarScore <= r1.bandarScore, 'Distribution harus turunkan bandarScore');
});

test('Retail panic naikkan bandarScore (peluang)', () => {
  const candles = makeCandles(25, { baseVol: 500000 });
  candles[24] = makePanicCandle(1000, 1500000);
  const r1 = analyzeBandar(makeCandles(25), {}, {}, {}, {});
  const r2 = analyzeBandar(candles, {}, {}, {}, {});
  assert(r2.bandarScore >= r1.bandarScore, 'Panic harus tidak menurunkan bandarScore');
});

test('stockType warning tidak null pada saham spekulatif', () => {
  const closes = [1000,1100,900,1200,800,1300,700,1400,600,1500,500,1300,600,1200,700,1100,800,1000,900,950];
  const candles = closes.map((c,i) => ({
    date:'2024-01-'+String(i+1).padStart(2,'0'),open:c,high:c+50,low:c-50,close:c,
    volume: i%3===0?5000000:100000
  }));
  const result = analyzeBandar(candles, {}, {}, { marketCap: 500e9 }, {});
  assert(result.stockType !== null);
  if (result.stockType.type === 'speculative') {
    assert(result.stockType.warning !== null, 'Speculative harus ada warning');
  }
});
