...
async function analyzeStock() {
  ...
  try {
    ...
    content.innerHTML = `<div style="text-align:center;padding:4rem 1rem"><div style="font-size:3rem;margin-bottom:1rem">⚠️</div><p style="font-size:1rem;color:const(--red);margin-bottom:8px;font-family:const(--mono);font-weight:700">Gagal menganalisis saham</p><p style="font-size:12px;color:const(--text3)">${esc(String(err.message))}</p></div>`;
...
function buildResult(ticker, d) {
  ...
  <div class="s-card" style="margin-bottom:10px">
    ...
    <!-- SKOR BAR -->
    <div style="border-top:1px solid const(--border);padding-top:1rem;margin-top:0.5rem">
      <div style="font-size:9px;color:const(--text3);font-family:const(--mono);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">SKOR SENTIMEN</div>
      <div class="skor-row">
        <div class="skor-num" style="color:${getScoreColor(finalScore)}">${finalScore}<span style="font-size:1rem;color:const(--text3)">/10</span></div>
    ...
    <div class="ticker-hdr">
      ...
      <div class="ticker-right"><div style="font-size:11px;color:const(--text3);margin-bottom:6px;font-family:const(--mono)">Data harga tidak tersedia</div><span class="sent-badge ${getBadgeClass(sentiment)}">${sentiment}</span></div>
    </div>
  </div>`;
    ...
    <div class="s-card" style="margin-bottom:0">
      ...
      <div style="margin-top:0.75rem">
        ${
          sc.breakdown
            ? `
        <div style="font-size:11px;color:const(--text3);margin-bottom:10px;font-family:const(--mono)">
          Trend ${sc.breakdown.trend?.score ?? '—'}/10 &nbsp;·&nbsp; Volume ${sc.breakdown.volume?.score ?? '—'}/10 &nbsp;·&nbsp; Momentum ${sc.breakdown.momentum?.score ?? '—'}/10
        </div>`
            : ''
        }
        <div style="font-size:11px;color:const(--text2);line-height:1.7">${esc(d.whyNow || d.summary || '—')}</div>
      </div>
    </div>
    <div class="s-card" style="margin-bottom:0">
      ...
      <div style="margin-top:0.75rem">
        ...
        <div style="font-size:11px;color:const(--text2);line-height:1.7;margin-top:4px">${esc(str.phaseLabel || vol.narrative || '—')}</div>
        ${str.trend ? `<div style="margin-top:6px;font-size:10px;color:const(--text3);font-family:const(--mono)">${trendIcon(str.trend?.direction)} ${esc(str.trend?.direction || '')} · ADX ${str.trend?.adx ?? '—'} · ${esc(str.trend?.strength || '')}</div>` : ''}
      </div>
    </div>
  </div>`;
  ...
  <div class="s-card">
    <div class="s-card-hdr">
      ...
      <div style="font-family:const(--mono);font-size:1.4rem;font-weight:700;color:${getScoreColor(finalScore)}">${finalScore}<span style="font-size:0.9rem;color:const(--text3)">/10</span></div>
    </div>
  ...
  <div class="two-col">
    <div class="s-card" style="margin-bottom:0">
      ...
      <div style="margin-top:0.75rem">${(ind.levels.support || []).map((l) => `<div class="sr-item" style="color:const(--g)">${fmtPrice(l)}</div>`).join('') || '<div style="color:const(--text3);font-size:12px">—</div>'}</div>
    </div>
    <div class="s-card" style="margin-bottom:0">
      <div class="s-card-label red">Resistance</div>
      <div style="margin-top:0.75rem">${(ind.levels.resistance || []).map((l) => `<div class="sr-item" style="color:const(--red)">${fmtPrice(l)}</div>`).join('') || '<div style="color:const(--text3);font-size:12px">—</div>'}</div>
    </div>
  </div>
  ...
  <div class="s-card">
    ...
    <div class="setup-card ${s.confidence}">
      <div class="setup-type" style="color:${s.direction === 'long' ? 'const(--g)' : s.direction === 'short' ? 'const(--red)' : 'const(--text2)'}">${esc(s.type?.replace(/_/g, ' '))} · ${esc(s.direction)} · ${esc(s.confidence)}</div>
  ...
  <div class="s-card">
    ...
    <div style="font-size:0.9rem;color:const(--text2);line-height:1.9;font-weight:400;margin-bottom:1rem">${esc(d.summary)}</div>
    <div class="rec-box">
  ...
  const analysisCards = !isIndex
    ? `
  <div class="two-col">
    <div class="s-card" style="margin-bottom:0"><div class="s-card-label blue">📈 Teknikal</div><div style="font-size:0.88rem;color:const(--text2);line-height:1.8;margin-top:0.75rem">${esc(d.analisisTeknikal)}</div></div>
    <div class="s-card" style="margin-bottom:0"><div class="s-card-label gold">📊 Fundamental</div><div style="font-size:0.88rem;color:const(--text2);line-height:1.8;margin-top:0.75rem">${esc(d.analisisFundamental)}</div></div>
  </div><div style="margin-bottom:10px"></div>`
    : `
  <div class="two-col">
  ...
  const kompetitifCard =
    !isIndex && d.posisiKompetitif
      ? `<div class="s-card" style="margin-bottom:10px"><div class="s-card-label blue">🏆 Posisi Kompetitif</div><div style="font-size:0.88rem;color:const(--text2);line-height:1.8;margin-top:0.75rem">${esc(d.posisiKompetitif)}</div></div>`
      : '';
  ...
  const sectorCtx =
    d.sektorContext && !isIndex
      ? `
  <div class="s-card"><div class="s-card-label blue">🔄 Konteks Sektor</div><div style="font-size:0.88rem;color:const(--text2);line-height:1.8;margin-top:0.75rem">${esc(d.sektorContext)}</div></div>`
      : '';
  ...
  <div class="s-card">
    ...
    <div style="margin-top:0.75rem;display:flex;flex-wrap:wrap;gap:6px">
      ${ind2.candlestick.patterns
        .map(
          (p) => `
      <div style="background:const(--bg3);border:1px solid ${p.type === 'bullish' ? 'rgba(0,230,118,0.2)' : p.type === 'bearish' ? 'rgba(255,82,82,0.2)' : 'const(--border)'};border-radius:9px;padding:0.7rem 0.9rem;flex:1;min-width:200px">
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:5px">
          <span style="font-size:11px;font-weight:700;font-family:const(--mono);color:${p.type === 'bullish' ? 'const(--g)' : p.type === 'bearish' ? 'const(--red)' : 'const(--text2)'}">${esc(p.name)}</span>
        ...
        <div style="font-size:11px;color:const(--text2);line-height:1.6">${esc(p.signal)}</div>
      </div>`,
        )
        .join('')}
    </div>
  ...
  <div class="s-card">
    ...
    <div style="margin-top:0.75rem">
      ...
      <div style="font-size:11px;color:const(--text2);margin-bottom:10px">${esc(ind2.fibonacci.narrative || '')}</div>
      <div class="stat-grid">
  ...
  <div class="two-col">
    ${
      ...
      <div style="margin-top:0.75rem">
        <div style="font-family:const(--mono);font-size:2rem;font-weight:700;color:${ind2.mfi.mfi < 30 ? 'const(--g)' : ind2.mfi.mfi > 70 ? 'const(--red)' : 'const(--gold)'}">${ind2.mfi.mfi}<span style="font-size:1rem;color:const(--text3)">/100</span></div>
        ...
        ${ind2.mfi.divergenceHint ? `<div style="font-size:11px;color:const(--gold);margin-top:6px">⚡ ${esc(ind2.mfi.divergenceHint.replace(/_/g, ' '))}</div>` : ''}
      </div>
    </div>`
        : '<div></div>'
    }
    ${
      ...
      <div style="margin-top:0.75rem">
        <div style="font-family:const(--mono);font-size:2rem;font-weight:700;color:${ind2.relStrength.rsScore >= 60 ? 'const(--g)' : ind2.relStrength.rsScore >= 40 ? 'const(--gold)' : 'const(--red)'}">${ind2.relStrength.rsScore}<span style="font-size:1rem;color:const(--text3)">/100</span></div>
        ...
        <div style="font-size:11px;color:const(--text2);margin-top:6px">${esc(ind2.relStrength.narrative || '')}</div>
      </div>
    </div>`
        : '<div></div>'
    }
    ...
    <div style="margin-top:0.75rem">
      ...
      ${(ind2.divergence.divergences || [])
        .map(
          (
            dv,
          ) => `<div style="font-size:11px;color:const(--text2);margin-top:5px;padding:5px 8px;background:const(--bg3);border-radius:6px">
        <span style="font-family:const(--mono);font-weight:700;color:${dv.type === 'bullish' ? 'const(--g)' : 'const(--red)'}">${dv.indicator}</span> — ${esc(dv.signal)}
      </div>`,
        )
        .join('')}
    </div>
  ...
  <div class="s-card">
    ...
    <div style="margin-top:0.75rem">
      <div style="font-size:11px;color:const(--text2);margin-bottom:10px;font-family:const(--mono)">${esc(
  ...
  <div class="s-card">
    <div class="s-card-hdr">
      ...
      <span style="font-size:9px;color:const(--text3);font-family:const(--mono)">Google News · CNBC · Detik · Kontan</span>
    </div>
    ${
      nd.emiten && nd.emiten.length
        ? `
    <div style="margin-bottom:1rem">
      <div style="font-size:9px;font-weight:700;color:const(--text3);text-transform:uppercase;letter-spacing:1.5px;font-family:const(--mono);margin-bottom:8px">📌 BERITA EMITEN</div>
      ${nd.emiten
        .map(
          (n) => `
      <div style="padding:8px 0;border-bottom:1px solid const(--border)">
        <div style="font-size:12px;font-weight:600;color:const(--text);line-height:1.5;margin-bottom:3px">${esc(n.title)}</div>
        ${n.description ? `<div style="font-size:11px;color:const(--text3);line-height:1.5">${esc(n.description)}</div>` : ''}
        <div style="font-size:10px;color:const(--text3);margin-top:3px;font-family:const(--mono)">${esc(n.source)} · ${esc(n.date)}</div>
      </div>`,
        )
        .join('')}
    </div>`
      ...
      nd.komoditas && nd.komoditas.length
        ? nd.komoditas
            .map(
              (c) => `
    <div style="margin-bottom:1rem">
      <div style="font-size:9px;font-weight:700;color:const(--gold);text-transform:uppercase;letter-spacing:1.5px;font-family:const(--mono);margin-bottom:8px">⛏️ ${esc(c.komoditas.toUpperCase())}</div>
      ${c.items
        .map(
          (n) => `
      <div style="padding:8px 0;border-bottom:1px solid const(--border)">
        <div style="font-size:12px;font-weight:600;color:const(--text);line-height:1.5">${esc(n.title)}</div>
        <div style="font-size:10px;color:const(--text3);margin-top:3px;font-family:const(--mono)">${esc(n.source)} · ${esc(n.date)}</div>
      </div>`,
        )
        .join('')}
    ...
    ${
      nd.makro && nd.makro.length
        ? `
    <div>
      <div style="font-size:9px;font-weight:700;color:const(--blue);text-transform:uppercase;letter-spacing:1.5px;font-family:const(--mono);margin-bottom:8px">🌏 SENTIMEN MARKET</div>
      ${nd.makro
        .map(
          (n) => `
      <div style="padding:8px 0;border-bottom:1px solid const(--border)">
  ...
  <div class="s-card">
    ...
    <table class="info-table" style="margin-top:0.75rem">
      ...
      ${hasNews ? `<tr><td>Berita</td><td style="color:const(--g)">✓ Tersedia</td></tr>` : `<tr><td>Berita</td><td style="color:const(--text3)">Tidak tersedia</td></tr>`}
    </table>
  </div>`;
...
async function runScanner() {
  ...
  res.innerHTML =
    '<div class="scanner-loading"><div style="font-size:13px;color:const(--text2);margin-bottom:8px;font-family:const(--mono)">Scanning ' +
    filterLabel +
    '...</div><div style="font-size:11px;color:const(--text3);margin-bottom:1rem;font-family:const(--mono)">Menganalisis 50+ saham IHSG</div><div class="progress-bar"><div class="progress-fill" id="scanProgress"></div></div></div>';
  ...
  try {
    ...
    res.innerHTML =
      ...
      esc(e.message) +
      '<br><span style="font-size:11px;color:const(--text3);margin-top:8px;display:block">Coba lagi beberapa saat.</span></div>';
...
function renderScanResults(data, filter) {
  ...
  if (!data.results || !data.results.length) {
    ...
    res.innerHTML = `<div class="scanner-empty">🔍 ${msg}<br><span style="font-size:11px;color:const(--text3);margin-top:8px;display:block">Coba filter lain atau scan ulang.</span></div>`;
    return;
  }
  ...
  <div class="scan-stats" style="margin-bottom:1rem">
    <div class="scan-stat"><span class="scan-stat-num" style="color:const(--g)">${data.results.length}</span><span class="scan-stat-lbl">Setup Ditemukan</span></div>
    <div class="scan-stat"><span class="scan-stat-num" style="color:const(--g)">${totalNaik}</span><span class="scan-stat-lbl">Naik</span></div>
    <div class="scan-stat"><span class="scan-stat-num" style="color:const(--red)">${totalTurun}</span><span class="scan-stat-lbl">Turun</span></div>
    <div class="scan-stat"><span class="scan-stat-num">${avgScore}</span><span class="scan-stat-lbl">Rata² Skor</span></div>
    <div class="scan-stat" style="margin-left:auto"><span style="font-size:10px;color:const(--text3);font-family:const(--mono)">dari ${data.universe} saham</span></div>
  </div>`;
  ...
  const cards = data.results
    .map((item) => {
      ...
      const scoreColor =
        item.score >= 7 ? 'const(--g)' : item.score >= 5 ? 'const(--gold)' : 'const(--red)';
      const scoreGrad =
        item.score >= 7
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
        <div style="display:flex;align-items:center;gap:10px">
          <div>
            <div style="font-family:const(--mono);font-size:1.1rem;font-weight:700;letter-spacing:-0.5px;line-height:1">${esc(item.ticker)}</div>
            <div style="font-size:10px;color:const(--text3);margin-top:2px;font-family:const(--mono)">${esc(item.sector || '')}</div>
          </div>
        </div>
        <div style="text-align:right">
          <div style="font-family:const(--mono);font-size:1.4rem;font-weight:700;color:${scoreColor};line-height:1">${item.score}<span style="font-size:0.75rem;color:const(--text3)">/10</span></div>
          <div style="font-size:9px;font-weight:700;font-family:const(--mono);color:${scoreColor};margin-top:1px">${esc(item.recommendation || '')}</div>
        </div>
      </div>
      <div style="font-size:12px;color:const(--text2);margin-bottom:8px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${esc(item.name || item.ticker)}</div>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
        <span style="font-family:const(--mono);font-size:1rem;font-weight:700">Rp ${item.lastClose ? item.lastClose.toLocaleString('id-ID') : 'N/A'}</span>
        <span style="font-size:11px;font-weight:700;font-family:const(--mono);color:${item.isUp ? 'const(--g)' : 'const(--red)'}">${item.isUp ? '+' : ''}${item.changePct || 0}%</span>
        ${item.rsi != null ? `<span style="font-size:10px;color:const(--text3);font-family:const(--mono);margin-left:auto">RSI <span style="color:${item.rsi < 30 ? 'const(--g)' : item.rsi > 70 ? 'const(--red)' : 'const(--text2)'};font-weight:700">${item.rsi}</span></span>` : ''}
      </div>
      <div style="height:2px;background:const(--bg3);border-radius:1px;overflow:hidden;margin-bottom:8px">
