...
function detectBandar(candles) {
  if (!candles || candles.length < 10) return null;
  try {
    const stealth    = detectStealthAccumulation(candles);
    const distTrap   = detectDistributionTrap(candles);
    const panic      = detectRetailPanic(candles);
    const smartMoney = detectSmartMoneyFootprint(candles, {}, {});
    const signal = stealth?.detected    ? 'stealth_accumulation'
                 : distTrap?.detected   ? 'distribution_trap'
                 : panic?.detected      ? 'retail_panic'
                 : smartMoney?.detected ? 'smart_money'
                 : null;
    return {
      signal,
      description: stealth?.description || distTrap?.description || panic?.description || smartMoney?.description || null,
      stealth, distTrap, panic, smartMoney
    };
  } catch(e) { return null; }
}
module.exports.detectBandar = detectBandar;
