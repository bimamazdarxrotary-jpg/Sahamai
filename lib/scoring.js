...
function scoreRisk(indicators, priceData) {
  let risk = 1; // base rendah — saham IDX secara umum volatile
  const reasons = [];
  ...
  // ATR volatilitas — threshold dinaikkan karena saham IDX memang volatile
  if (atrData?.atrPct > 8)        { risk += 3; reasons.push(`Volatilitas ekstrem (ATR ${atrData.atrPct}%)`); }
  else if (atrData?.atrPct > 5)   { risk += 2; reasons.push(`Volatilitas sangat tinggi (ATR ${atrData.atrPct}%)`); }
  else if (atrData?.atrPct > 3)   { risk += 1; reasons.push(`Volatilitas tinggi (ATR ${atrData.atrPct}%)`); }
  else                             { reasons.push('Volatilitas normal'); }
...
// Risk diintegrasikan sebagai penalty multiplier — bukan komponen additive —
// karena scoreRisk pakai skala terbalik (makin tinggi = makin berbahaya).
const WEIGHT_TREND    = 0.28; // Trend: 28%
const WEIGHT_VOLUME   = 0.22; // Volume: 22%
const WEIGHT_MOMENTUM = 0.28; // Momentum: 28%
const WEIGHT_SETUP    = 0.22; // Setup: 22%

// Risk penalty max 30%: risk=0→no penalty | risk=5→-15% | risk=10→-30%
const RISK_PENALTY_MAX = 0.15; // max 15% penalty — lebih proporsional untuk saham IDX
...
function computeScore(indicators, volumeData, structure, priceData) {
  ...
  // Risk penalty: risk=3→-9% | risk=6→-18% | risk=9→-27%
  ...
  return {
    ...
    breakdown: {
      ...
      risk:     { score: riskS.score,     weight: RISK_PENALTY_MAX, penaltyApplied: parseFloat(((1 - riskPenaltyFactor) * 100).toFixed(1)) + '%', reasons: riskS.reasons },
