...
const MODEL    = 'openai/gpt-oss-120b'; // Model terbaik di Groq per Mei 2026 — pengganti Maverick
...
[INDIKATOR TAMBAHAN]
• MFI(14): ${mfi.mfi != null ? mfi.mfi + ' (' + (mfi.signal || '') + ')' : 'N/A'} | Divergence: ${div.detected ? div.summary : 'Tidak ada'} | Candlestick: ${cs.topPattern ? cs.topPattern.name + ' (' + cs.topPattern.type + ')' : 'N/A'}
• Fibonacci: ${fib.narrative || 'N/A'}${fib.atKeyLevel ? ' ⚠️ LEVEL KUNCI' : ''} | RS: ${rs.label || 'N/A'} | Pivot: ${pivots.narrative || 'N/A'}
`;
}
...
function buildStockPrompt(ticker, metadata, priceContext, technicalContext, scoring) {
  ...
  const sektorContext = getSektorContext(sektorResmi, subsektor);
  const bandaContext  = getBandarContext(scoring, metadata);

  return `Kamu adalah Senior Equity Analyst IHSG dengan pengalaman 20+ tahun di Bursa Efek Indonesia.

KEAHLIAN: Bandar IHSG, foreign flow, MSCI rebalancing, sector rotation, commodity linkage (CPO/batubara/nikel), smart money behavior.
...
function getSektorContext(sektor, subsektor) {
  const konteks = {
    'Energi': `KONTEKS: Sektor Energi sensitif harga komoditas (batubara Newcastle, CPO, minyak). Foreign ownership tinggi, korelasi USDIDR kuat. Waspadai DMO/windfall tax.`,

    'Perbankan': `KONTEKS: Perbankan dominasi asing (BBCA/BBRI/BMRI masuk MSCI). Sensitif BI Rate, NPL, NIM. Pemotongan BI Rate = katalis positif.`,

    'Teknologi': `KONTEKS: Tech IHSG (GOTO/BUKA) masih burn rate tinggi. Valuasi GMV-based, sensitif Nasdaq & Fed Rate. Retail dominan, rawan panic sell.`,

    'Konsumer': `KONTEKS: Konsumer defensif, outperform risk-off. Sensitif inflasi CPO/gandum, daya beli. Musiman Lebaran/Natal signifikan.`,

    'Properti': `KONTEKS: Properti sensitif KPR/BI Rate, siklus panjang. Stimulus PPN 0% = katalis. Likuiditas kecil rawan manipulasi.`,

    'Infrastruktur': `KONTEKS: Infrastruktur BUMN sensitif APBN/PSN. IKN = katalis. Saham politik, utang tinggi, cash flow berisiko.`,

    'Barang Baku': `KONTEKS: Barang Baku terkait LME (nikel, tembaga, emas). TPIA sensitif naphtha/petrokimia. Sangat cyclical, demand China driven.`,

    'Telekomunikasi': `KONTEKS: TLKM barometer market, dividen yield tinggi. Kompetisi ketat tekan ARPU. 5G rollout = katalis menengah.`,

    'Kesehatan': `KONTEKS: Kesehatan defensif, valuasi premium. BPJS/INA-CBG pengaruhi margin RS. Post-COVID normalisasi volume layanan.`,
  };

  const key = Object.keys(konteks).find(function(k) {
...
function getBandarContext(scoring, metadata) {
  if (!scoring) return '';
  const lines = ['PANDUAN ANALISIS BANDAR & SMART MONEY:'];

  if (scoring.breakdown && scoring.breakdown.volume) {
    const vs = scoring.breakdown.volume.score;
  ...
  if (scoring.breakdown && scoring.breakdown.trend) {
    const ts = scoring.breakdown.trend.score;
    if (ts >= 8) lines.push(`• Trend score sangat kuat (${ts}/10): Kemungkinan ada penggerak institusional di belakang tren ini`);
  }

  lines.push('• Cek volume spike: body besar = genuine move, doji = absorpsi bandar. OBV naik saat harga turun = smart money akumulasi.');
...
KEAHLIAN: Makro Indonesia (BI Rate/inflasi/rupiah), foreign flow, MSCI rebalancing, sector rotation IHSG, risk-on/off global.
...
async function callAI(params) {
  ...
  // Tambah konteks berita ke prompt
  const newsContext = newsData && newsData.summary
    ? '\n\n' + newsData.summary
    : '\n\nBERITA TERKINI: Tidak tersedia saat ini — gunakan pengetahuan terkini yang kamu miliki.';
  ...
  const res = await fetchWithRetry(GROQ_URL, {
    ...
    body: JSON.stringify({
      model:       MODEL,
      max_tokens:  5000, // 30+ field JSON butuh ruang lebih
      temperature: 0.15, // rendah = output deterministik & konsisten, hindari hallucination
      messages: [
        {
          role:    'system',
  ...
  // Beberapa model Groq kadang output thinking tags <think>...</think> — strip dulu
