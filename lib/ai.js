...
function getBandarContext(scoring, bandarData) {
  const hints = [];
  ...
  // Sertakan signals dari smartMoneyFootprint jika ada
  if (bandarData && bandarData.smartMoney && bandarData.smartMoney.signals && bandarData.smartMoney.signals.length) {
    bandarData.smartMoney.signals.slice(0, 3).forEach(function(sig) {
      hints.push(sig);
    });
  }
  ...
  return hints.length ? 'BANDAR HINTS:\n' + hints.map(h => '• ' + h).join('\n') : '';
}
...
{
  "whyNow": "BBCA saat ini berada di atas EMA9 (9.850) dan SMA50 (9.600) dengan RSI 58 di zona momentum positif. Volume akumulasi 7 dari 10 hari terakhir menunjukkan smart money masih menahan posisi. Breakout dari resistance 10.000 belum terkonfirmasi namun momentum mendukung.",
  "sentiment": "BELI",
...
PERHATIKAN: scoreTeknikal WAJIB sesuai data scoring. Angka EMA9, SMA50, RSI, target, SL harus masuk akal dan konsisten satu sama lain.
`;

// ── deriveSentimentFromScore ────────────────────────────────────────
// Satu sumber kebenaran untuk memetakan scoring.recommendation (deterministik)
// ke sentiment (BELI/TAHAN/JUAL). Dipakai baik untuk instruksi prompt AI
// maupun untuk enforcement hard-override di api/analyze.js — sebelumnya
// mapping ini hanya ada di prompt (instruksi lunak "WAJIB") tanpa ada
// pemaksaan di level kode, sehingga AI yang tidak patuh bisa menghasilkan
// sentiment yang bertentangan dengan skor teknikal tanpa terdeteksi.
function deriveSentimentFromScore(scoring) {
  if (!scoring) return 'TAHAN';
  if (scoring.recommendation === 'BELI' || scoring.recommendation === 'AKUMULASI') return 'BELI';
  if (scoring.recommendation === 'JUAL' || scoring.recommendation === 'KURANGI')   return 'JUAL';
  return 'TAHAN';
}

// ── buildStockPrompt ──────────────────────────────────────────────
function buildStockPrompt(ticker, metadata, priceContext, technicalContext, scoring, bandarData) {
  ...
  const recSentiment = deriveSentimentFromScore(scoring);

  const sektorContext = getSektorContext(sektorResmi);
  const bandarContext = getBandarContext(scoring, bandarData);
...
function sanitizeAIOutput(parsed, priceData, indicators) {
  ...
  // Arah rekomendasi menentukan validasi target/SL — JUAL punya arah kebalikan dari BELI/TAHAN.
  // (Bug fix: sebelumnya target & SL SELALU divalidasi seolah bullish, sehingga untuk saham
  //  ber-sentiment JUAL, target/SL yang benar-benar bearish dari AI malah ditimpa jadi bullish.)
  const isBearish = (parsed.sentiment === 'JUAL');

  if (isBearish) {
    // Target JUAL: harus di BAWAH current (downside target), tidak kurang dari 50% current
    if (target !== null && (target >= current || target < current * 0.5)) {
      const est = current - targetBuffer;
      parsed.targetHarga = 'Rp ' + est.toLocaleString('id-ID') + ' (estimasi ATR)';
    }
    // SL JUAL: harus di ATAS current (invalidasi tesis bearish), tidak lebih dari 25% di atas
    if (sl !== null && (sl <= current || sl > current * 1.25)) {
      const est = current + slBuffer;
      parsed.stopLoss = 'Rp ' + est.toLocaleString('id-ID') + ' (estimasi ATR)';
    }
  } else {
    // Target BELI/TAHAN: harus di atas current, tidak lebih dari 2x current
    if (target !== null && (target <= current || target > current * 2)) {
      const est = current + targetBuffer;
      parsed.targetHarga = 'Rp ' + est.toLocaleString('id-ID') + ' (estimasi ATR)';
    }
    // SL BELI/TAHAN: harus di bawah current, tidak lebih dari 25% di bawah
  ...
  // Entry ("levelBeli"): untuk BELI/TAHAN ini zona beli aktif; untuk JUAL ini zona re-entry
  // setelah koreksi — keduanya wajar berada di/bawah current, jadi validasi batas atas tetap sama.
  ...
  // Entry zone warning — redaksi berbeda untuk JUAL (re-entry) vs BELI/TAHAN (buy zone)
  if (entry !== null && current > entry * 1.02) {
    if (isBearish) {
      parsed.entryWarning = 'Harga saat ini (Rp ' + current.toLocaleString('id-ID') + ') masih jauh dari zona re-entry Rp ' + (entry - entryBuffer).toLocaleString('id-ID') + ' - Rp ' + entry.toLocaleString('id-ID') + '. Tunggu konfirmasi reversal sebelum masuk kembali.';
    } else {
...
async function callAI(params) {
  ...
  const prompt = isIndex
    ...
    : buildStockPrompt(ticker, metadata, priceContext, technicalContext + newsContext, scoring, bandarData);
...
module.exports = { callAI, sanitizeAIOutput, deriveSentimentFromScore };
