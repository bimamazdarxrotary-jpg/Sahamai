...
function buildTechnicalContext(indicators, volumeData, structure, scoring, bandarData, priceData, marketContext) {
  const i  = indicators  || {};
  const v  = volumeData  || {};
  const s  = structure   || {};
  const sc = scoring     || {};
...
MA20: ${i.ma && i.ma.ma20 ? i.ma.ma20.toLocaleString('id-ID') : 'N/A'} | MA50: ${i.ma && i.ma.ma50 ? i.ma.ma50.toLocaleString('id-ID') : 'N/A'} | EMA9: ${i.ma && i.ma.ema9 ? i.ma.ema9.toLocaleString('id-ID') : 'N/A'}
...
TREND: ${maStatus} | ADX=${i.trend ? i.trend.adx : 'N/A'} (${i.trend ? i.trend.strength : 'N/A'}) | HH/HL=${s.hhll ? s.hhll.pattern.toUpperCase() : 'N/A'} | RS=${rs.rsScore ? rs.rsScore + '/100' : 'N/A'}
...
VOLUME: Bias=${v.accDist ? v.accDist.bias.toUpperCase() : 'N/A'} (acc=${v.accDist ? v.accDist.accDays : 0}h/10h) | OBV=${i.obv ? i.obv.trend.toUpperCase() : (v.obv ? v.obv.trend : 'N/A')} | VWAP=${v.vwap ? v.vwap.toLocaleString('id-ID') : 'N/A'} | SMF=${v.smartMoneyFlow ? v.smartMoneyFlow.ratio + '% (' + v.smartMoneyFlow.bias + ')' : 'N/A'}
