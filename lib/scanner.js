...
function quickScan(ticker, candles, indicators, volumeData, structure, scoring, changePct, cacheGet) {
  ...
  if (ind.ma && ind.ma.type === 'golden_cross' && isLongOk) {
    signals.push({
      ...
      direction: 'long',
      strength:  'high',
      reason:    'EMA9 memotong ke atas SMA50 — sinyal uptrend kuat'
    });
  }
  if (ind.ma && ind.ma.type === 'death_cross') {
    signals.push({
      ...
      direction: 'short',
      strength:  'high',
      reason:    'EMA9 memotong ke bawah SMA50 — sinyal downtrend kuat'
    });
  }
  ...
  // ── 8. RVOL + AKUMULASI — lebih akurat dari MFI ──────────────────
  // RVOL >= 1.5x dengan bias akumulasi = sinyal kuat smart money masuk
  const rvol = ind.rvol;
  if (rvol && rvol.rvol >= 1.5 && isLongOk) {
    const isAcc  = vol.accDist && vol.accDist.bias === 'accumulation';
    const obvRising = ind.obv && ind.obv.trend === 'rising';
    const alreadyHasVolSpike = signals.some(function(s) { return s.type === 'volume_spike'; });
    if ((isAcc || obvRising) && !alreadyHasVolSpike) {
      signals.push({
        type:      'volume_spike',
        label:     'Volume Abnormal',
        direction: 'long',
        strength:  rvol.rvol >= 3 ? 'high' : 'medium',
        reason:    'RVOL ' + rvol.rvol + 'x (' + rvol.label + ') dengan ' +
                   (isAcc ? 'pola akumulasi' : 'OBV rising') + ' — volume di atas normal mengkonfirmasi buying pressure'
      });
    }
  } else if (rvol && rvol.rvol >= 2) {
    // Volume spike besar tapi arah tidak jelas — tetap informasikan
    const alreadyHasVolSpike = signals.some(function(s) { return s.type === 'volume_spike'; });
    if (!alreadyHasVolSpike) {
      signals.push({
        type:      'volume_spike',
        label:     'Volume ' + rvol.rvol + 'x',
        direction: 'neutral',
        strength:  'medium',
        reason:    'RVOL ' + rvol.rvol + 'x rata-rata (' + rvol.label + ') — pantau konfirmasi arah'
      });
    }
  }

  // ── 8b. SMART MONEY FLOW ─────────────────────────────────────────
  // Konfirmasi tambahan dari smartMoneyFlow di indicators (bukan volumeData)
  const smf = ind.smartMoney;
  if (smf && smf.bias === 'strong_buying' && isLongOk) {
    const alreadyHasSmf = signals.some(function(s) { return s.type === 'volume_spike' && s.direction === 'long'; });
    if (!alreadyHasSmf) {
      signals.push({
        type:      'volume_spike',
        ...
        direction: 'long',
        strength:  'medium',
        reason:    'Smart money flow bullish kuat (' + smf.ratio + '%) — pelaku besar dominan di sisi beli'
      });
    }
  } else if (smf && smf.bias === 'strong_selling') {
    const alreadyHasDist = signals.some(function(s) { return s.direction === 'short' && s.type === 'volume_spike'; });
    if (!alreadyHasDist) {
      signals.push({
        type:      'volume_spike',
        ...
        direction: 'short',
        strength:  'medium',
        reason:    'Smart money flow bearish kuat (' + smf.ratio + '%) — tekanan jual dari pelaku besar'
      });
    }
  }
