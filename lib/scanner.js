...
function quickScan(ticker, candles, indicators, volumeData, structure, scoring) {
  ...
  // ── NEW: Ready to Pump composite signal ─────────────────────
  // Kombinasi: score tinggi + RSI belum overbought + ada sinyal bullish + akumulasi
  var bullishSignals = signals.filter(function(s) { return s.direction === 'long'; });
  var rsiVal = ind.rsi;
  var hasAccum = vol.accDist && vol.accDist.bias === 'accumulation';
  var scoreVal = sc.final || 0;
  var notDowntrend = !str.trend || str.trend.direction !== 'downtrend' || str.trend.strength === 'weak' || str.trend.strength === 'no_trend';

  if (bullishSignals.length >= 2 && rsiVal != null && rsiVal < 45 && rsiVal > 10 && scoreVal >= 6 && notDowntrend) {
    var reasons = bullishSignals.slice(0, 2).map(function(s) { return s.label; }).join(' + ');
    if (hasAccum) reasons += ' + Akumulasi';
    signals.unshift({  // taruh di depan agar jadi topSignal
      type:      'ready_pump',
      label:     '🎯 Ready to Pump',
      direction: 'long',
      detail:    'Setup matang untuk naik: ' + reasons + '. RSI ' + rsiVal + ', skor ' + scoreVal + '/10.',
