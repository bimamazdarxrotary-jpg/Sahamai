  ...
  try {
    ...
    for (const line of dataLines) {
      ...
      candles.push({
        ...
        open:   Math.round(iOpen  >= 0 ? parseFloat(parts[iOpen])  || close : close),
        high:   Math.round(iHigh  >= 0 ? parseFloat(parts[iHigh])  || close : close),
        low:    Math.round(iLow   >= 0 ? parseFloat(parts[iLow])   || close : close),
        close:  Math.round(close),
        volume: iVol >= 0 ? parseInt(parts[iVol]) || 0 : 0
      });
    }
