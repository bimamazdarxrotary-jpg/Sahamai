...
function detectUnusualActivity(candles, period = 20) {
  ...
  for (const c of recent) {
    ...
    const bodyPct  = c.high !== c.low ? Math.abs(c.close - (c.open || c.close)) / (c.high - c.low) : 0;
    ...
    if (isSpike) {
      flags.push({
        date:    c.date,
        type:    isAbsorb ? 'absorption' : c.close > (c.open || c.close) ? 'buying_climax' : 'selling_climax',
...
function detectClimaxVolume(candles) {
  if (!candles || candles.length < 21) return null;
  const spike = detectVolumeSpike(candles, 3);
  if (!spike?.isSpike) return { isClimax: false };

  const last = candles[candles.length - 1];
  const prev = candles[candles.length - 2];

  // Climax: volume ekstrim + reversal candle
  const isReversal = (last.close < prev.close && last.close < last.open)  // bearish setelah uptrend
                  || (last.close > prev.close && last.close > last.open);   // bullish setelah downtrend

  return {
    isClimax:   spike.isSpike,
...
function smartMoneyFlow(candles, period) {
  ...
  var recent = candles.slice(-period);
  var bullFlow = 0, bearFlow = 0;

  recent.forEach(function(c) {
    var open    = c.open || c.close;
    var range   = c.high - c.low || 1;
    // Smart money biasanya push harga di close
    var smfMult = ((c.close - c.low) - (c.high - c.close)) / range;
    var flow    = smfMult * c.volume;
    if (flow > 0) bullFlow += flow;
    else          bearFlow += Math.abs(flow);
  });

  var total = bullFlow + bearFlow;
  var ratio = total > 0 ? parseFloat((bullFlow / total * 100).toFixed(1)) : 50;
...
function volumePriceTrend(candles) {
  ...
  for (var i = 1; i < candles.length; i++) {
    var pctChange = candles[i-1].close ? (candles[i].close - candles[i-1].close) / candles[i-1].close : 0;
    vpt += pctChange * candles[i].volume;
    vptSeries.push(vpt);
  }

  var recent5 = vptSeries.slice(-5);
  var older5  = vptSeries.slice(-10, -5);
  var recentAvg = recent5.reduce(function(a,b){return a+b;},0) / 5;
  var olderAvg  = older5.length ? older5.reduce(function(a,b){return a+b;},0) / older5.length : recentAvg;
...
function analyzeVolume(candles) {
  ...
  var smf = smartMoneyFlow(candles, 14);
  var vpt = volumePriceTrend(candles);
  ...
  return {
    ...
    vpt:          vpt,
    narrative,
    score:        computeVolumeScore(spike, accDist, confirmation, climax)
  };
}
 ...
 * Score volume 0–10
 */
function computeVolumeScore(spike, accDist, confirmation, climax) {
