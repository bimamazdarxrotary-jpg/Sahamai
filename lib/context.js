...
function analyzeSectorStrength(sektor, candles) {
  ...
  var lastClose  = recent[recent.length - 1].close;
  var returnPct  = firstClose ? ((lastClose - firstClose) / firstClose * 100) : 0;
...
function analyzeSectorRotation(sektor, riskSentiment) {
  ...
  var mode    = riskSentiment ? riskSentiment.mode : 'neutral';
  var mapping = rotationMap[mode] || rotationMap['neutral'];

  var isBeneficiary = mapping.beneficiary.some(function(s) {
    return sektor && sektor.toLowerCase().includes(s.toLowerCase());
  });
  var isLaggard = mapping.laggard.some(function(s) {
    return sektor && sektor.toLowerCase().includes(s.toLowerCase());
  });
...
function analyzeMarketContext(ticker, candles, indicators, volumeData, structure) {
  var sektor         = getSektorFromTicker(ticker);
  var sectorStrength = analyzeSectorStrength(sektor, candles);
  var riskSentiment  = detectRiskSentiment(indicators, volumeData, structure);
  var sectorRotation = analyzeSectorRotation(sektor, riskSentiment);

  // Phase summary
  var phase = structure ? structure.phase : 'unknown';
  var marketPhase = {
    phase:   phase,
