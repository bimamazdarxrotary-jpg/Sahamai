...
async function analyzeSectorStrength(sektor, candles, ticker) {
  ...
  // Fallback 1: sektor return dari cache scanner (data peer nyata, di-set oleh api/scanner.js)
  if (return20d === null) {
    const sectorCache = cacheGet('sector:returns');
    if (sectorCache && sectorCache[sektor] && sectorCache[sektor].avg != null) {
      return20d = sectorCache[sektor].avg;
      source    = 'scanner_cache';
    }
  }

  // Fallback 2: proxy dispersion dari pergerakan saham sendiri
  if (return20d === null) {
  ...
  return {
    ...
    note:  source === 'idx_index'
      ...
      : source === 'scanner_cache'
        ? 'Berdasarkan data peer dari hasil scan terbaru'
 ...
 * v4: tambah smartMoney dan OBV sebagai sinyal tambahan
 */
function detectRiskSentiment(indicators, volumeData, structure) {
  ...
  if (indicators && indicators.ma) {
    if (indicators.ma.alignment === 'bullish') { score += 1; signals.push('MA bullish alignment'); }
    else                                        { score -= 1; signals.push('MA bearish alignment'); }
  }

  // Smart Money Flow — v4: sinyal dari indicators.smartMoney
  if (indicators && indicators.smartMoney) {
    const smf = indicators.smartMoney;
    if      (smf.bias === 'strong_buying')  { score += 1; signals.push('Smart money flow bullish kuat (' + smf.ratio + '%)'); }
    else if (smf.bias === 'mild_buying')    { signals.push('Smart money flow cenderung beli'); }
    else if (smf.bias === 'strong_selling') { score -= 1; signals.push('Smart money flow bearish kuat (' + smf.ratio + '%)'); }
    else if (smf.bias === 'mild_selling')   { signals.push('Smart money flow cenderung jual'); }
  }

  // OBV trend — konfirmasi volume
  if (indicators && indicators.obv) {
    if (indicators.obv.trend === 'rising')  { score += 1; signals.push('OBV rising — akumulasi'); }
    if (indicators.obv.trend === 'falling') { score -= 1; signals.push('OBV falling — distribusi'); }
  }

  let mode;
  if      (score >= 4)  mode = 'risk_on';
  else if (score >= 2)  mode = 'mild_risk_on';
  else if (score >= -1) mode = 'neutral';
  else if (score >= -3) mode = 'mild_risk_off';
  else                  mode = 'risk_off';

  return { mode, score, signals, label: getRiskLabel(mode) };
}
...
function buildContextSummary(sektor, sectorStrength, riskSentiment, sectorRotation) {
  ...
  if (riskSentiment) {
    ...
    // Highlight sinyal SMF jika ada
    const smfSignal = riskSentiment.signals.find(function(s) {
      return s.includes('Smart money');
    });
    if (smfSignal) parts.push(smfSignal);
  }
  if (sectorStrength && sectorStrength.strength !== 'unknown') {
    ...
    const src = sectorStrength.source === 'idx_index'     ? ' [IDX index]'
              : sectorStrength.source === 'scanner_cache' ? ' [peer data]'
              : '';
    parts.push('Sektor ' + sektor + ': ' + sectorStrength.label + ' (return 20d: ' + ret + '%' + src + ')');
  }
  if (sectorRotation) {
