...
async function fetchSectorReturn(sectorIndex) {
  ...
  try {
    ...
    const timer = setTimeout(function() { ctrl.abort(); }, SECTOR_FETCH_TIMEOUT);
 ...
 *
 * CATATAN FIELD:
 * - return20d  : return 20 hari (field utama, dipakai test & UI)
 * - sectorReturn: alias return20d untuk backward compat
 */
async function analyzeSectorStrength(sektor, candles, ticker) {
  ...
  if (return20d === null) {
    ...
    return20d = dispersion > 15 && Math.abs(selfReturn) > 8
      ? selfReturn * 0.6
      : selfReturn;
  ...
  let strength;
  if      (return20d > 10)  strength = 'very_strong';
  else if (return20d > 5)   strength = 'strong';
  else if (return20d > 0)   strength = 'neutral_positive';
  else if (return20d > -5)  strength = 'neutral_negative';
  else if (return20d > -10) strength = 'weak';
  ...
  return {
    ...
    strength,
    return20d,
    sectorReturn: return20d,  // alias backward compat
    selfReturn,
    source,
 ...
 *
 * Matching menggunakan exact match dulu, fallback ke partial includes.
 * Ini memastikan 'Konsumer Non-Primer' tidak false-match ke 'Konsumer Primer'.
 */
function analyzeSectorRotation(sektor, riskSentiment) {
  ...
  const rotationMap = {
    ...
    'mild_risk_on': {
      beneficiary: ['Keuangan', 'Energi', 'Infrastruktur', 'Industri', 'Perindustrian'],
    ...
    'mild_risk_off': {
      ...
      laggard:     ['Energi', 'Properti', 'Teknologi', 'Perindustrian', 'Industri']
    },
    'risk_off': {
      beneficiary: ['Konsumer Primer', 'Konsumer Non-Primer', 'Kesehatan'],
  ...
  // Match: exact dulu, lalu partial (case-insensitive)
  function matchesList(list, sektor) {
    // Exact match
    if (list.indexOf(sektor) !== -1) return true;
    // Partial: sektor mengandung salah satu entry list
    return list.some(function(s) {
      return sektor.toLowerCase().includes(s.toLowerCase()) ||
             s.toLowerCase().includes(sektor.toLowerCase());
    });
  }

  const isBeneficiary = matchesList(mapping.beneficiary, sektor);
  const isLaggard     = !isBeneficiary && matchesList(mapping.laggard, sektor);
 ...
 * Async karena analyzeSectorStrength melakukan fetch ke Yahoo Finance
 */
...
function buildContextSummary(sektor, sectorStrength, riskSentiment, sectorRotation) {
  ...
  if (sectorStrength && sectorStrength.strength !== 'unknown') {
    const ret = sectorStrength.return20d != null ? sectorStrength.return20d : sectorStrength.sectorReturn;
...
// ── Wrapper baru untuk api/analyze.js (signature simpel) ──────────
// fetchMarketContext(ticker, sector) — tanpa butuh candles/indicators/structure
async function fetchMarketContext(ticker, sector) {
  const sektor = sector || getSektorFromTicker(ticker);
  if (!sektor || sektor === 'Indeks') return null;

  // Fetch sector return dari indeks IDX
  const sectorIdx    = SECTOR_INDEX_MAP[sektor] || null;
  const return20d    = sectorIdx ? await fetchSectorReturn(sectorIdx) : null;
  ...
  // Risk sentiment sederhana dari IHSG cache
  const ihsgChg   = require('./cache').cacheGet('ihsg:changePct');
  let marketRisk  = 'neutral';
  if      (ihsgChg != null && ihsgChg > 1)   marketRisk = 'risk_on';
  else if (ihsgChg != null && ihsgChg > 0)   marketRisk = 'mild_risk_on';
  else if (ihsgChg != null && ihsgChg < -2)  marketRisk = 'risk_off';
  else if (ihsgChg != null && ihsgChg < 0)   marketRisk = 'mild_risk_off';

  // Sector bias
  const rotationMap = {
    'risk_on':       { ben: ['Energi','Barang Baku','Teknologi','Properti','Industri','Perindustrian'], lag: ['Konsumer Primer','Konsumer Non-Primer','Kesehatan'] },
    'mild_risk_on':  { ben: ['Keuangan','Energi','Infrastruktur','Industri'], lag: ['Teknologi','Properti'] },
    'neutral':       { ben: ['Keuangan','Konsumer Primer','Konsumer Non-Primer','Infrastruktur'], lag: [] },
    'mild_risk_off': { ben: ['Konsumer Primer','Konsumer Non-Primer','Kesehatan'], lag: ['Energi','Properti','Teknologi'] },
    'risk_off':      { ben: ['Konsumer Primer','Konsumer Non-Primer','Kesehatan'], lag: ['Energi','Barang Baku','Teknologi','Properti'] }
  };
  const rm = rotationMap[marketRisk] || rotationMap['neutral'];
  const isBen = rm.ben.some(s => sektor.includes(s) || s.includes(sektor));
  const isLag = !isBen && rm.lag.some(s => sektor.includes(s) || s.includes(sektor));
  const sectorBias = isBen ? 'beneficiary' : isLag ? 'laggard' : 'neutral';

  return {
    sectorName:      sektor,
    sectorReturn20d: return20d,
    sectorStrength:  strength,
    sectorLabel:     getStrengthLabel(strength),
    marketRisk,
    sectorBias,
    summary: 'Sektor ' + sektor + ': ' + getStrengthLabel(strength) + ' | Market: ' + marketRisk + ' | Bias: ' + sectorBias
  };
}

// Tambah ke exports
module.exports.fetchMarketContext = fetchMarketContext;
