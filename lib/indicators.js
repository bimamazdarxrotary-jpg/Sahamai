...
function candlestickPatterns(candles) {
  ...
  // Bug fix: fallback sebelumnya (substitusi last.close saat last.open null) ternyata
  // membuat kondisi engulfing SELALU mustahil terpenuhi secara matematis untuk KEDUA pola
  // (bukan cuma Bearish — Bullish juga terdampak, terverifikasi lewat pengujian langsung).
  // Alasan: begitu last.open disubstitusi close, kedua syarat inequality saling
  // bertentangan (mis. untuk Bullish: syarat 1 mengharuskan last.close > prev.close,
  // syarat 2 versi fallback mengharuskan last.close < prev.close — kontradiksi).
  ...
  const hasOpenData = last.open != null && prev.open != null;

  if (hasOpenData && green(last) && red(prev) &&
  ...
  if (hasOpenData && red(last) && green(prev) &&
      last.close < prev.open &&
      last.open  > prev.close &&
      body(last) > body(prev) * 1.1)
  ...
  if (red(prev2) && body(prev) < body(prev2) * 0.5 && green(last) &&
      last.close > (prev2.close + (prev2.open != null ? prev2.open : prev2.close)) / 2 && isDn)
  ...
  if (green(prev2) && body(prev) < body(prev2) * 0.5 && red(last) &&
      last.close < (prev2.close + (prev2.open != null ? prev2.open : prev2.close)) / 2 && isUp)
