...
const { hasOpen, candleBody, isGreenCandle, isRedCandle, upperWick, lowerWick } = require('./candleUtils');
...
// Bug fix: sebelumnya hanya Engulfing yang dijaga dari kasus `open` null.
// Hammer/Shooting Star/Marubozu/Morning Star/Evening Star masih pakai
// fallback lokal yang membuat body candle jadi 0 secara diam-diam saat
// open null — Marubozu jadi mustahil terdeteksi, Hammer/Shooting Star
// rawan false-positive, Morning/Evening Star salah klasifikasi arah.
// Sekarang semua pola pakai lib/candleUtils.js yang eksplisit skip
// (bukan menebak) saat data open tidak tersedia.
function candlestickPatterns(candles) {
  ...
  // Hammer (setelah downtrend) — butuh open last untuk body/wick akurat
  if (lastBody != null && lastLower > lastBody * 2 && lastUpper < lastBody * 0.5 && isDn)
  ...
  if (lastBody != null && lastUpper > lastBody * 2 && lastLower < lastBody * 0.5 && isUp)
  ...
  // Bug fix (sebelumnya): fallback substitusi last.close saat last.open null membuat
  // kondisi engulfing SELALU mustahil terpenuhi secara matematis untuk KEDUA pola.
  // Pola engulfing memang butuh open asli dari kedua candle untuk valid secara definisi;
  // kalau data open tidak tersedia, opsi paling benar adalah SKIP deteksi, bukan menebak.
  const prevBody    = candleBody(prev);
  const hasOpenData = hasOpen(last) && hasOpen(prev);

  if (hasOpenData && isGreenCandle(last) && isRedCandle(prev) &&
  ...
  if (hasOpenData && isRedCandle(last) && isGreenCandle(prev) &&
      last.close < prev.open &&
      last.open  > prev.close &&
      lastBody > prevBody * 1.1)
  ...
  // Morning Star (setelah downtrend) — butuh open di prev2, prev, dan last
  const prev2Body   = candleBody(prev2);
  const hasOpenStar = hasOpen(prev2) && hasOpen(prev) && hasOpen(last);

  if (hasOpenStar && isRedCandle(prev2) && prevBody < prev2Body * 0.5 && isGreenCandle(last) &&
      last.close > (prev2.close + prev2.open) / 2 && isDn)
  ...
  if (hasOpenStar && isGreenCandle(prev2) && prevBody < prev2Body * 0.5 && isRedCandle(last) &&
      last.close < (prev2.close + prev2.open) / 2 && isUp)
    pats.push({ name: 'Evening Star', type: 'bearish', strength: 'high',
      signal: 'Evening Star setelah uptrend — reversal bearish 3 candle' });

  // Marubozu
  if (lastBody != null && lastBody / range(last) > 0.92) {
    const bull = isGreenCandle(last);
  ...
  // Inside Bar — tidak bergantung pada open sama sekali, aman seperti semula
