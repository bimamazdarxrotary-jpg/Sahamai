...
// ── Wrapper untuk api/analyze.js ──────────────────────────────────
async function fetchNewsData(ticker, sector, subsector) {
  const metadata = { sector: sector || null, subsector: subsector || null };
  const isIndex  = ticker === 'IHSG' || ticker === '^JKSE';
  try {
    const [emitenNews, commNews, ihsgNews] = await fetchAllNews(ticker, metadata, isIndex);
    // Tambah field sentiment sederhana ke setiap berita
    const addSentiment = (arr) => (arr || []).map(n => {
      const text  = (n.title + ' ' + (n.summary || '')).toLowerCase();
      const posKw = ['naik','tumbuh','profit','laba','positif','rekor','ekspansi','kuat','meningkat','bagus'];
      const negKw = ['turun','rugi','merosot','anjlok','negatif','bangkrut','gagal','melemah','kontraksi'];
      const posHit = posKw.filter(k => text.includes(k)).length;
      const negHit = negKw.filter(k => text.includes(k)).length;
      return { ...n, sentiment: posHit > negHit ? 'positive' : negHit > posHit ? 'negative' : 'neutral' };
    });
    return {
      emiten: addSentiment(emitenNews),
      commodity: addSentiment(commNews),
      ihsg: addSentiment(ihsgNews)
    };
  } catch (e) {
    return { emiten: [], commodity: [], ihsg: [] };
  }
}

module.exports.fetchNewsData = fetchNewsData;
