// ══════════════════════════════════════════════════════════════════
// lib/structure.js — Market Structure Engine v4
// Sesuai indicators.js v4: EMA9/SMA50, tanpa ADX, tanpa relStrength
// Deteksi tren, breakout, HH/HL, setup, struktur market IHSG
// ══════════════════════════════════════════════════════════════════
...
function detectSetup(candles, indicators, volumeData) {
  ...
  // ── PULLBACK SETUP ───────────────────────────────────────────────
  // Pullback ke EMA9 dalam uptrend (harga di atas SMA50, dekat EMA9 ±2%)
  if (maData?.aboveSMA50 && maData?.ema9 && last.close < maData.ema9 * 1.02 && last.close > maData.ema9 * 0.98) {
    setups.push({
      type:       'pullback',
      direction:  'long',
      confidence: 'medium',
      reason:     `Pullback ke EMA9 (${maData.ema9?.toLocaleString('id-ID')}) dalam uptrend — zona beli potensial.`
    });
  }
  ...
  if (maData?.type === 'golden_cross') {
    setups.push({
      ...
      direction:  'long',
      confidence: 'high',
      reason:     `Golden Cross EMA9 menembus SMA50 — sinyal uptrend kuat.`
    });
  }
  ...
  if (maData?.type === 'death_cross') {
    setups.push({
      ...
      direction:  'short',
      confidence: 'high',
      reason:     `Death Cross EMA9 di bawah SMA50 — sinyal downtrend kuat.`
    });
  }
 ...
 * v4: pakai EMA9/SMA50/alignment dari maCrossover(), tanpa ADX
 * Kekuatan tren diturunkan dari bullPct (confidence sinyal)
 */
function analyzeTrend(candles, indicators) {
  ...
  // HH/HL pattern (2 bobot)
  if (hhll.pattern === 'uptrend')   bullSignals += 2;
  if (hhll.pattern === 'downtrend') bearSignals += 2;

  // EMA9 / SMA50 position — hanya dihitung jika maData tersedia
  if (maData) {
    if (maData.aboveEMA9)  bullSignals++; else bearSignals++;
    if (maData.aboveSMA50) bullSignals++; else bearSignals++;
    if (maData.alignment === 'bullish') bullSignals++;
    if (maData.alignment === 'bearish') bearSignals++;
  }
  ...
  // Kekuatan tren diturunkan dari bullPct — pengganti ADX yang dihapus
  const confidence = Math.round(Math.abs(bullPct - 50) * 2); // 0–100
  const strength   = confidence >= 70 ? 'very_strong'
...
function analyzeStructure(candles, indicators, volumeData) {
  ...
  // Market phase — pakai strength dari trend (diturunkan dari bullPct, bukan ADX)
  let phase;
  if (breakout?.isBreakout && breakout.confirmed) {
    phase = breakout.type === 'bullish_breakout' ? 'markup' : 'markdown';
  } else if (trend?.direction === 'uptrend' && (trend?.strength === 'strong' || trend?.strength === 'very_strong')) {
    phase = 'markup';
  } else if (trend?.direction === 'downtrend' && (trend?.strength === 'strong' || trend?.strength === 'very_strong')) {
    phase = 'markdown';
  ...
  // Phase confidence 0–100 — pakai trend.confidence (diturunkan dari bullPct)
  ...
  else if (trend?.confidence >= 70) phaseConfidence = 80;
  else if (trend?.confidence >= 50) phaseConfidence = 70;
  else if (trend?.confidence < 10)  phaseConfidence = 30;
