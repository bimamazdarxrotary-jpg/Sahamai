...
// Karakter yang tidak boleh ada di ticker
const BLACKLIST_CHARS = /[^A-Z0-9]/g;

function validateTicker(raw) {
  if (!raw || typeof raw !== 'string')
    return { valid: false, error: 'Kode saham tidak boleh kosong.' };

  // Strip whitespace + karakter non-alphanumeric + uppercase
  const clean = raw.trim().toUpperCase().replace(BLACKLIST_CHARS, '');

  if (!clean)
    return { valid: false, error: 'Kode saham tidak valid.' };
  if (clean.length < 2)
    return { valid: false, error: `Kode saham terlalu pendek: "${clean}".` };
  if (clean.length > 6)
    return { valid: false, error: `Kode saham terlalu panjang: "${clean}".` };
  if (!VALID_TICKER_PATTERN.test(clean) && !INDEX_TICKERS.has(clean))
    return { valid: false, error: `Format tidak valid: "${clean}". Gunakan kode saham IDX (contoh: BBCA, TLKM).` };
...
function validateAIOutput(raw) {
  ...
  // Normalize confidenceLevel ↔ confidence — AI kadang pakai salah satu
  if (parsed.confidenceLevel && !parsed.confidence) {
    parsed.confidence = parsed.confidenceLevel;
  } else if (parsed.confidence && !parsed.confidenceLevel) {
    parsed.confidenceLevel = parsed.confidence;
  }
  // Normalize nilai confidence ke High/Medium/Low
  const confMap = { high: 'High', medium: 'Medium', low: 'Low', tinggi: 'High', sedang: 'Medium', rendah: 'Low' };
  const confRaw = (parsed.confidence || parsed.confidenceLevel || '').toLowerCase();
  const confNorm = confMap[confRaw] || (parsed.confidence || 'Medium');
  parsed.confidence = confNorm;
  parsed.confidenceLevel = confNorm;

  // scoreTeknikal — pastikan formatnya "<angka> - <label>"
  if (parsed.scoreTeknikal) {
    const scoreStr = String(parsed.scoreTeknikal);
    // Jika AI hanya kirim angka, tambah dash dan label
    if (/^\d+$/.test(scoreStr.trim())) {
      parsed.scoreTeknikal = scoreStr.trim() + '/10';
    }
  }
